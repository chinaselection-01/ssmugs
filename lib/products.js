...
// English / Polish product "core phrase" (style + type, deduplicated) for SEO names.
// Replaces the old per-style composition to avoid awkward doubles like "Round Tumbler Tumbler".
const CORE_EN = {
  pastoral:'Pastoral Bottle', straight:'Straight Bottle', diamond:'Diamond Bottle', gourd:'Gourd Bottle',
  milk:'Milk Bottle', cola:'Cola Bottle', belly:'Round Tumbler', narrow:'Narrow-mouth Bottle',
  singlewall:'Single-wall Bottle', vacuum:'Cola Bottle', wide:'Wide-mouth Bottle', car:'Car Cup',
  lid:'Bottle Lid', can:'Can-style Bottle', coffee:'Coffee Cup', egg:'Egg Cup', temp:'Temp-display Bottle',
  threein1:'3-in-1 Bottle', bullet:'Bullet Bottle', waist:'Slim Waist Bottle',
};
const CORE_PL = {
  pastoral:'Butelka Rustykalna', straight:'Butelka Prosta', diamond:'Butelka Romb', gourd:'Butelka Dyniowa',
  milk:'Butelka Mleczna', cola:'Butelka Cola', belly:'Kubek Okrągły', narrow:'Butelka Wąskoustna',
  singlewall:'Butelka Jednościenna', vacuum:'Butelka Cola', wide:'Butelka Szerokoustna', car:'Kubek Samochodowy',
  lid:'Pokrywka do Butelki', can:'Butelka Puszkowa', coffee:'Kubek Kawowy', egg:'Kubek Jajkowy', temp:'Butelka z Termometrem',
  threein1:'Butelka 3-w-1', bullet:'Butelka Pociskowa', waist:'Butelka Wąska Talia',
};

// Real, fact-based insulation descriptor (all drinkware is insulated; vacuum style is vacuum-sealed).
function insulationEN(styleKey) { return styleKey === 'vacuum' ? 'Vacuum Insulated' : 'Double Wall Insulated'; }
function insulationPL(styleKey) { return styleKey === 'vacuum' ? 'Próżniowo Izolowana' : 'Izolowana Podwójna Ścianka'; }

// slugify an EN product title into an ASCII filename (used for image naming)
function slugify(s) {
...
// Injects B2B marketing terms: stainless steel, insulated/vacuum/double wall,
// wholesale, factory, OEM/ODM, competitive price, bulk order.
function buildSeo(p) {
  const cap = p.capacityMl ? `${p.capacityMl}ml ` : '';
  let coreEn = CORE_EN[p.styleKey] || 'Bottle';
  let corePl = CORE_PL[p.styleKey] || 'Butelka';
  // The two plastic-lid variants share styleKey 'lid' but are distinct products — differentiate titles.
  if (p.styleKey === 'lid') {
    if (p.name === '直杯塑料盖') { coreEn = 'Straight-cup Lid'; corePl = 'Pokrywka do Prostego Kubka'; }
    else if (p.name === '腰型塑料盖') { coreEn = 'Waist Lid'; corePl = 'Pokrywka Talii'; }
  }
  // The three car-cup generations share styleKey 'car' — differentiate by generation.
  if (p.styleKey === 'car') {
    const gen = p.name.startsWith('3代') ? 'Gen 3' : p.name.startsWith('2代') ? 'Gen 2' : 'Gen 1';
    coreEn = `Car Cup ${gen}`;
    corePl = `Kubek Samochodowy ${gen}`;
  }
  const insEn = insulationEN(p.styleKey);
  const insPl = insulationPL(p.styleKey);
  const enName = `${cap}Stainless Steel ${insEn} ${coreEn} Wholesale Factory OEM ODM`.trim();
  const plName = `${cap}Stal Nierdzewna ${insPl} ${corePl} Hurtownia Fabryka OEM ODM`.trim();
  const enTitle = `${enName} | Competitive Price Bulk Order B2B China`;
  const plTitle = `${plName} | Konkurencyjna Cena Zamówienie Hurtowe B2B Chiny`;
  const enDesc = `Wholesale ${enName.toLowerCase()} from China stainless steel factory. Competitive price, bulk order, OEM/ODM custom logo, B2B supply, low MOQ, fast delivery. Direct-sale insulated drinkware for distributors and private-label brands.`;
  const plDesc = `Hurtowa ${plName.toLowerCase()} z chińskiej fabryki stali nierdzewnej. Konkurencyjna cena, zamówienie hurtowe, OEM/ODM z własnym logo, zaopatrzenie B2B, niskie MOQ, szybka dostawa.`;
  return {
...
export const products = RAW.map((r, i) => {
  ...
  const base = {
    ...
    meas,
    whiteBox,
    // price adjustment: all ex-work prices +¥1 per piece (per request)
    unitPrice: unitPrice == null ? null : unitPrice + 1,
